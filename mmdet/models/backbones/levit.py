...
def b16(n, activation, resolution=224):
    return torch.nn.Sequential(
        ...
        activation(),
        Conv2d_BN(n // 2, n, 3, 1, 1, resolution=resolution // 8))
...
class Attention(torch.nn.Module):
    def __init__(self, dim, key_dim, num_heads=8,
        ...
        points = list(itertools.product(
            range((resolution + 1) // 2), range((resolution + 1) // 2)))
        N = len(points)
        attention_offsets = {}
        idxs = []
        for p1 in points:
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        attn += torch.nn.functional.interpolate(
            self.attention_biases[:, self.attention_bias_idxs].unsqueeze(1)
            if self.training else self.ab.unsqueeze(1),
            size=attn.size()[-2:]).squeeze()
        # attn = (
...
class AttentionSubsample(torch.nn.Module):
    def __init__(self, in_dim, out_dim, key_dim, num_heads=8,
        ...
        self.resolution = resolution
        points = list(itertools.product(
            range(resolution // 2), range(resolution // 2)))
        points_ = list(itertools.product(
            range(resolution_ // 2), range(resolution_ // 2)))
        N = len(points)
        N_ = len(points_)
        attention_offsets = {}
        idxs = []
...
class LeViT(torch.nn.Module):
    ...
    def __init__(self, img_size=224,
        ...
        for i, (ed, kd, dpth, nh, ar, mr, do) in enumerate(
            ...
            self.block_idxs.append(len(self.blocks))
...
class LeViTWrapper(LeViT):

    def __init__(self, C, D, X, N, drop_path, weights,
                 num_classes=1000, distillation=True, pretrained=False):
        ...
        self.blocks = torch.nn.ModuleList([
                          self.blocks[pre:next] for pre, next in
                          zip([0] + self.block_idxs[:-1], self.block_idxs)])
        self.proj = torch.nn.ModuleList([
                        torch.nn.Conv2d(128, 192, 1),
                        torch.nn.Conv2d(256, 384, 1),
                        torch.nn.Conv2d(768, 768, 1)])

    def forward(self, x):
        xs = super().forward(x)
        xs = xs[:2] + [torch.cat(xs[-2:], dim=2)]

        outs = []
        for x, p in zip(xs, self.proj):
            x = x.transpose(1, 2)
            shape_ = list(x.size())[:-1] + [int(math.sqrt(x.size()[-1]))] * 2
            x = x.reshape(*shape_)
            outs.append(p(x))
        return outs
